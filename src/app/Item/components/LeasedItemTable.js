...
import { useGetAllLeasedItems } from '../hooks/useFetchItems';
import useLeasedItemColumnGenerator from './LeasedItemColumnGenerator';
import SearchBar from '../../Common/components/Search/SearchBar';

const LeasedItemTable = () => {
  const { column } = useLeasedItemColumnGenerator();
  ...
  const { items, pagination, loading } = useGetAllLeasedItems(query);
...
export default LeasedItemTable;
